...
import { UserRole } from '@boilerplate/types';
import { logObjs, logger } from '@boilerplate/utils';
import { corsify } from '..';
...
router.post(
  '/check',
  // withAuth(),
  (req: Request, res: Response, env: Env, ctx: ExecutionContext) => healthCheckJson(req, env)
);
router.get(
  '/check2',
  // withAuth(),
  (req: IRequest, res: Response, env: Env, ctx: ExecutionContext) => healthCheckJson(req, env)
);
router.get(
  '/check-root',
  // withAuth(),
