...
import { UseFetchResult, FetchError } from '.'
...
const useFetchCopy = async <T>(
  ...
  const headers =
    IS_SSR && process.env.NODE_ENV === 'development'
      ? {
  ...
  try {
    ...
    if (FETCH_DEBUG) {
      ...
      ct === 'text/plain' &&
        logger.debug(`[ui] [useFetch] response: ${JSON.stringify(await response.clone().text())}`)
    }
